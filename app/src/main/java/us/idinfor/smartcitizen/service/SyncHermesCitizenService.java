...
public class SyncHermesCitizenService extends Service {

    private static final String TAG = SyncHermesCitizenService.class.getCanonicalName();
    ...
    public SyncHermesCitizenService() {}

    public static void startActionQueryAll(Context context) {
        Intent intent = new Intent(context, SyncHermesCitizenService.class);
    ...
    private void handleApiResult(String action, Integer result) {
        ...
        switch (result) {
            ...
            case HermesCitizenApi.RESPONSE_ERROR_USER_NOT_FOUND:
                //Toast.makeText(getApplicationContext(), "Error: User not found", Toast.LENGTH_LONG).show();
            ...
            case HermesCitizenApi.RESPONSE_ERROR_DATA_NOT_UPLOADED:
                //Toast.makeText(getApplicationContext(), "Error: Data not uploaded", Toast.LENGTH_LONG).show();
    ...
    public void onFitDataResult(FitDataSetsResultEvent result){
        switch (result.getQueryType()){
            case GoogleFitService.QUERY_LOCATIONS_HERMES:
                ...
                handleActionUploadLocations(username, HermesCitizenSyncUtils.dataSetsToLocationSampleList(result.getDataSets()));
                break;
        }
    }
    ...
    public void onFitDataResult(FitBucketsResultEvent result){
        switch (result.getQueryType()){
            case GoogleFitService.QUERY_ACTIVITIES_HERMES:
                ...
                /*startActionUploadActivities(
                        getApplicationContext(),
                        username,
                        HermesCitizenSyncUtils.bucketsToActivitySegmentList(result.getBuckets())
                );*/
                String username = Utils.getSharedPreferences(getApplicationContext()).getString(Constants.PROPERTY_USER_NAME,"");
                handleActionUploadActivities(username, HermesCitizenSyncUtils.bucketsToActivitySegmentList(result.getBuckets()));
                break;
        }
    }
    ...
    private static PendingIntent getSyncPI(Context context) {
        if (syncPI == null) {
            Intent syncService = new Intent(context, SyncHermesCitizenService.class);
