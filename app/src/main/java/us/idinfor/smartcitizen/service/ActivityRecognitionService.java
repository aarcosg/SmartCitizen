...
public class ActivityRecognitionService extends Service implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener,
    ...
    private Integer mCurrentActivity;
    ...
    private void handleActionActivityRecognitionResult(ActivityRecognitionResult result){
        ...
        mCurrentActivity = detectedActivity.getType();
        int confidence = detectedActivity.getConfidence();
        Log.d(TAG, "Detected context=" + Constants.getActivityString(this,mCurrentActivity) + " Confidence=" + confidence);
        ...
        if(confidence >= Constants.MIN_DETECTED_CONFIDENCE) {
            ...
            if (deviceId > 0L) {
                ...
                prefs.edit()
                        .putInt(Constants.PROPERTY_LAST_DETECTED_ACTIVITY, mCurrentActivity)
                        .apply();
            }
        }
    }
    ...
    private void sendContextToBackend(){
        mCurrentActivity = prefs.getInt(Constants.PROPERTY_LAST_DETECTED_ACTIVITY,-1);
        if(mCurrentActivity >= 0){
            Log.d(TAG, "Send new context to backend: " + mCurrentActivity);
            if(mLastLocation != null){
                Log.d(TAG, "Location !=null: Lat=" + mLastLocation.getLatitude() + " Lon=" + mLastLocation.getLongitude());
            }
            if(Utils.isInternetAvailable(this)){
                new AddDetectedContextAsyncTask(deviceId,mCurrentActivity,mLastLocation){
                    @Override
